...
import { Form, Button, Container, Row, Col, Card, Alert, Image, Spinner } from 'react-bootstrap';
...
const VendorProfileEdit = () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setFormData({
            ...
            categories: profile.categories || ['Türk Mutfağı'],
  ...
  const uploadImage = async (file, type) => {
    try {
      ...
      console.log('Resim yükleme isteği gönderiliyor:', {
        endpoint: 'http://localhost:5000/api/vendors/upload',
        type: type,
      ...
      if (error.response) {
        ...
        if (error.response.status === 400) {
          throw new Error(`Geçersiz istek (400): ${error.response.data?.message || 'Dosya formatı veya boyutu kabul edilmedi'}`);
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Satıcı profili güncelleniyor:', vendorData);
      ...
      if (response.data && response.data.success) {
        ...
        setError(response.data?.message || 'Satıcı profili güncellenirken bir hata oluştu');
      }
    } catch (error) {
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <div className="d-grid gap-2 mt-4">
                  <Button 
                    variant="primary" 
                    type="submit"
                    disabled={updating || success || uploadingImages}
                  >
                    {updating ? 'Güncelleniyor...' : uploadingImages ? 'Resimler Yükleniyor...' : 'Profili Güncelle'}
                  </Button>
